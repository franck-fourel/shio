	...
	return this.doToServersAndUpdate(
		...
		function(server, listing) {
			...
			if (listing.state === 'RUNNING') {
				fns.push(client.stop.bind(client, id));
			}

			fns.push(client.load.bind(client, id, body.binary, body.config));

			if (! params.noStart) {
				fns.push(client.start.bind(client, id));
			}
...
SlotActions.prototype.doToServersAndUpdate = function(params, fn, cb) {
	...
	async.parallel(subActions, function(err) {
			...
			for (var server in servers) {
				updateActions.push(self.sitter.updateListingsForServer.bind(self.sitter, name))
			}
			async.parallel(updateActions, function(ignoredErr) {
				if (err != null) {
					if (typeof err === 'error') {
						console.warn("Error starting some processes.");
						console.log(err.stack);
						return cb("Unable to start all processes.");
					} else {
						return cb(err);
					}
				}
