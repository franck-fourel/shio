...
var getKey = function(elem) {
  ...
  if (host == null || port == null) {
    throw except.IAE("Must specify a host and port property, got[%j]", elem);
  }
...
module.exports = function(selfDescription, config, gossipClientFactory, polling) {
  ...
  var retVal = {
    ...
    addAgent: function(agent) {
      ...
      var port = agent['port'];
      if (host == null || port == null) {
        throw except.IAE("Must specify a host and port property, got[%j]", agent);
      }
  ...
  polling.repeat(
      ...
      resyncClient.getCoordinators(function(err, coords){
        if (err == null) {
          for (var i = 0; i < coords.length; ++i) {
            ...
            if (key !== selfKey && coordinators[key] == null) {
    ...
    config.resyncPollDuration || config.pollDuration * 10
  );
