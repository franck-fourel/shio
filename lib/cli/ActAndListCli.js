...
ActAndListCli.prototype.doAction = function(payload, cb) {
  ...
  if (this.client == null) {
    ...
    this.lifecycle.add('jsonClient', jsonClient);
...
ActAndListCli.prototype.doActionAndShow = function(payload) {
  ...
  this.lifecycle.add('readLine', rl);
  ...
  return async.waterfall(
    [ this.showAction.bind(this),
      function(results, cb) {
        ...
        console.log("This command will effect:");
        self._displayResults(results);
...
ActAndListCli.prototype.build = function(commandFn) {
  this.program
    ...
    .option('-P, --port', 'specify the coordinator port.  Overrides SHIO_COORDINATOR_PORT, defaults to 17000')
