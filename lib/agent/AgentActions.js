...
AgentActions.prototype.load = function(params, bundle, res, cb) {
  ...
  var nextDeployCount = this.currDeployCount(slot) + 1;
...
AgentActions.prototype.start = function(params, bundle, res, cb) {
  ...
  var startScript = path.join(deployDir, util.format('%s-%s', bundle.binary.name, bundle.binary.version), 'start.sh');
  ...
  var child = child_process.spawn(startScript, [], { detached: true, stdio: [ 'ignore', out, err]})
...
AgentActions.prototype.expandTar = function(tarFile, outDir, cb) {
  ...
  child_process.exec(util.format('tar xzf %s', tarFile), {cwd: outDir}, function(err, stdout, stdin) {
    if (err) {
      console.error("Unable to untar[%s]", tarFile);
    }
    return cb(err);
  });
}
...
AgentActions.prototype.addListing = function(slot) {
  ...
  if (! fs.existsSync(bundleFile)) {
    console.warn("Expected bundleFile[%s] to exist, but it didn't.  Skipping.", bundleFile);
    return;
  } 
